...
_Why?_ Without `await`, you're comparing a Promise object, not the actual result.
...
_Why?_ While this works, `async`/`await` is clearer and more maintainable.
...
_Why?_ Real delays slow down your test suite. Use fake timers instead.
...
_Fix:_ Either `await` the promise or `return` it.
