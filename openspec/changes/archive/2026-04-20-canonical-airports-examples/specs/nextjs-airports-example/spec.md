...
- **THEN** use `src/` with subdirectories following definitive-react.md import hierarchy: `data-access/`, `utils/`, `hooks/`, `stores/`, `modules/`, `features/`, `components/`

#### Scenario: Feature folders with five-file pattern

- **WHEN** organizing data-fetching features
- **THEN** use five-file pattern in `features/`: `features/airports-layer/`, `features/airports-list/`, `features/airports-card/` with `index.tsx`, `server.tsx`, `client.tsx`, `loading.tsx`, `error.tsx`

#### Scenario: Pure UI components

- **WHEN** organizing components without data fetching
