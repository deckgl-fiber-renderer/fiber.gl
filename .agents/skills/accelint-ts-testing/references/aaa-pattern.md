...
_Why?_ Without clear separation and with multiple behaviors tested together, it's hard to understand what's being tested and why a test fails.
...
_Why?_ Without visual separation, it's harder to quickly identify where setup ends and the actual test logic begins.
...
_Why?_ Each test should verify one behavior. Split this into three separate tests: creation, update, and deletion.
...
_Why?_ If the test has bugs, you won't know if the test or the code is wrong. Keep tests simple and obvious.
...
_Why?_ Complex setup obscures the test's purpose. Extract to helper functions or test-utils.
...
_Why?_ Duplicated setup code makes tests harder to maintain and increases the chance of inconsistencies. Extract common setup into a function that returns test fixtures. This also allows you to add useful utilities (like `advanceToNextSecond()`) that make tests more readable.
...
_Why?_ Multiple async operations without clear separation make it unclear what's being tested.
