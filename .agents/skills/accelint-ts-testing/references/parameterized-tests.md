...
_Why?_ Duplicated test code is harder to maintain. If the assertion logic changes, you need to update multiple tests.
...
_Why?_ When a test fails, you won't know which input caused the failure without checking test output details.
...
describe("calculateDiscount", () => {
  ...
  ])("should throw for invalid inputs: price=$price, discount=$discount", ({ price, discount }) => {
...
_Why?_ Conditional logic in tests makes them harder to understand and maintain. Each test should have one clear purpose.
...
describe("validateEmail", () => {
  it.each(["test@example.com", "user.name@example.co.uk", "user+tag@example.com"])(
...
describe("validatePhone", () => {
  it.each(["123-456-7890", "(123) 456-7890", "+1-123-456-7890"])(
    "should return true for valid phone: %s",
    (phone) => {
...
_Why?_ When setup or assertions differ significantly, separate tests are clearer than conditional logic within a parameterized test.
