...
_Why?_ Testing only that an error is thrown doesn't verify you're throwing the right error with the right message.
...
_Why?_ Any error will pass this test, including unexpected errors from bugs in your code.
...
_Why?_ If the function doesn't throw, the test will pass incorrectly. Use `rejects` matcher instead.
...
describe("calculateAge", () => {
  ...
  ])("should throw for invalid birthdate: $birthdate", ({ birthdate, error }) => {
...
describe("DataService", () => {
  ...
  it("should fail after max retries", async () => {
    ...
    await expect(service.fetchWithRetry("/api/data")).rejects.toThrow("Max retries exceeded");
    expect(apiClient.fetch).toHaveBeenCalledTimes(3);
  });
});
```
...
describe("UserRepository", () => {
  it("should handle database connection errors", async () => {
    ...
    await expect(repo.findById("user-123")).rejects.toThrow("Database error: Connection lost");
  });
  ...
  it("should handle query timeouts", async () => {
    ...
    await expect(repo.findById("user-123")).rejects.toThrow("Query timeout");
  });
});
```
...
describe("processFile", () => {
  ...
  ])('should reject dangerous filename: "$filename"', ({ filename, error }) => {
...
describe("processPayment", () => {
  it("should provide helpful error for insufficient funds", async () => {
    ...
    await expect(processPayment(payment)).rejects.toThrow(
      "Insufficient funds: balance $100, required $1000",
    );
  });
  ...
  it("should include transaction ID in error messages", async () => {
    ...
    await expect(processPayment(payment)).rejects.toThrow(expect.stringContaining("txn-123"));
  });
});
```
...
_Why incorrect?_ This test assumes TypeScript prevents invalid inputs. In production, `processUser` might receive `null`, `undefined`, or `{ name: 'Bob' }` (missing email) from:
...
describe("processUser", () => {
  ...
  it("should handle null user", () => {
    expect(() => processUser(null as any)).toThrow("User cannot be null or undefined");
  });

  it("should handle undefined user", () => {
    expect(() => processUser(undefined as any)).toThrow("User cannot be null or undefined");
  });

  it("should handle missing name field", () => {
    const user = { email: "alice@example.com" };

    expect(() => processUser(user as any)).toThrow("User name is required");
  });

  it("should handle missing email field", () => {
    const user = { name: "Alice" };

    expect(() => processUser(user as any)).toThrow("User email is required");
  });

  it("should handle empty object", () => {
    expect(() => processUser({} as any)).toThrow("User name is required");
  });
});
```

_Why correct?_ These tests verify the function has defensive runtime validation. The `as any` casts are intentional—they simulate real runtime scenarios where type safety is bypassed.
...
describe("parseUserResponse", () => {
  ...
  it("should reject response missing required fields", () => {
    ...
    expect(() => parseUserResponse(json)).toThrow("Email is required");
  });

  it("should reject malformed JSON", () => {
    const json = "{invalid json}";

    expect(() => parseUserResponse(json)).toThrow("Invalid JSON");
  });

  it("should reject null response", () => {
    const json = "null";

    expect(() => parseUserResponse(json)).toThrow("Response cannot be null");
  });

  it("should reject response with wrong types", () => {
    const json = '{"id": "not-a-number", "name": "Alice", "email": "alice@example.com"}';

    expect(() => parseUserResponse(json)).toThrow("ID must be a number");
  });
});
```
...
describe("processExternalData", () => {
  it("should handle library returning null", () => {
    ...
    expect(() => processExternalData(mockLib)).toThrow("Library returned null data");
  });

  it("should handle library returning unexpected structure", () => {
    const mockLib = { getData: () => ({ wrongField: "value" }) };

    expect(() => processExternalData(mockLib)).toThrow("Invalid data structure");
  });
  ...
  it("should handle library throwing non-Error objects", () => {
    ...
    expect(() => processExternalData(mockLib)).toThrow("Unexpected error type");
  });
});
```
...
it("should reject null input", () => {
  expect(() => processData(null as any)).toThrow("Input cannot be null");
});

// Testing JSON parsing scenarios
it("should handle API returning wrong type", () => {
  const response = { id: "string-instead-of-number" };
  expect(() => validate(response as any)).toThrow("ID must be number");
});
```
...
describe("createOrder", () => {
  ...
  it("should create order with valid data", () => {
    const validOrder = {
      customerId: "c-123",
      items: [{ id: "i-1", quantity: 2, price: 10.0 }],
    };

    const order = createOrder(validOrder);

    expect(order.total).toEqual(20.0);
  });
  ...
  it.each([
    ...
    expect(() => createOrder(data as any)).toThrow(`${field} is required`);
  });
  ...
  it.each([
    ...
    {
      data: { customerId: "c-123", items: "not-array" },
      field: "items",
      expectedType: "array",
    },
  ])("should reject wrong type for $field", ({ data, field, expectedType }) => {
    expect(() => createOrder(data as any)).toThrow(`${field} must be ${expectedType}`);
  });
  ...
  it("should reject empty items array", () => {
    ...
    expect(() => createOrder(order)).toThrow("Order must contain at least one item");
  });

  it("should reject negative quantities", () => {
    const order = {
      customerId: "c-123",
      items: [{ id: "i-1", quantity: -1, price: 10.0 }],
    };
